...
interface SellerOrderEmailParams {
  sellerName: string;
  buyer: BuyerInfo;
  shippingInfo: ShippingInfo | null;   // may not always be stored in DB
  items: OrderItem[];                  // only this seller's items
  orderId: number;
  orderTotal: number;                  // total for this seller's items only
}

export function buildSellerOrderEmail(params: SellerOrderEmailParams): string {
