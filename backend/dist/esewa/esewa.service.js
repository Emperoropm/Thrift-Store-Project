...
const esewa_config_1 = require("./esewa.config");
...
class EsewaService {
    ...
    async createPaymentRequest(orderId, totalAmount) {
        try {
            ...
            const message = `total_amount=${totalAmountInPaisa},transaction_uuid=${transactionId},product_code=${esewa_config_1.esewaConfig.merchantId}`;
            const signature = crypto_1.default
                .createHmac('sha256', esewa_config_1.esewaConfig.secretKey)
                .update(message)
                .digest('base64');
            ...
            const paymentData = {
                ...
                product_code: esewa_config_1.esewaConfig.merchantId,
            ...
            return {
                paymentUrl: `${esewa_config_1.esewaConfig.baseUrl}/api/epay/main/v2/form`,
    ...
    async verifyPaymentFromCallback(data) {
        try {
            // eSewa sends data as form-urlencoded
            const { transaction_uuid, transaction_code, status, total_amount, signature } = data;
            if (!transaction_uuid) {
            ...
            if (signature) {
                ...
                const verifyMessage = `total_amount=${total_amount},transaction_uuid=${transaction_uuid},product_code=${esewa_config_1.esewaConfig.merchantId}`;
            ...
            throw new Error('Failed to verify payment from callback: ' + error.message);
        }
    }
    ...
    async verifyPaymentManually(transactionId, orderId) {
        try {
            // Verify with eSewa API
            const verifyUrl = `${esewa_config_1.esewaConfig.baseUrl}/api/epay/transaction/status/${transactionId}`;
            console.log('Verifying payment with URL:', verifyUrl);
            const response = await axios_1.default.get(verifyUrl, {
                headers: {
                    'Authorization': `Bearer ${esewa_config_1.esewaConfig.secretKey}`,
